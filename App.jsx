...
function StoreBuilderModal({ workspace, onClose }) {
  ...
  const [config, setConfig] = useState(() => {
    try {
      ...
      if (saved) {
        return {
          ...saved,
          sections: saved.sections?.length ? saved.sections : [],
          bundles: saved.bundles || [],
          gallery: saved.gallery || [],
          selectedProducts: saved.selectedProducts || [],
          selectedCollections: saved.selectedCollections || [],
          shipping: saved.shipping || { mode: "global", globalLocal: Number(workspace.frais_livraison || 0), globalShipping: Number(workspace.frais_expedition || 0), products: {} },
          header: saved.header || { showSearch: true, showCart: true, showMenu: true, sticky: true },
          footer: saved.footer || { phone: workspace.whatsapp_number || "", whatsapp: workspace.whatsapp_number || "", facebook: workspace.facebook_url || "", instagram: workspace.instagram_url || "", tiktok: workspace.tiktok_url || "", links: ["Livraison", "Retours", "Confidentialité"] },
        };
      }
    } catch {}
    return {
      ...
      sections: [
        ...
        { id: "catalog", type: "catalog", label: "Catalogue produits", enabled: true, height: 460, radius: 18, padding: 22 },
        ...
        { id: "promo", type: "promo", label: "Promotion", enabled: true, height: 220, radius: 18, padding: 22 },
        { id: "reviews", type: "reviews", label: "Avis clients", enabled: true, height: 220, radius: 18, padding: 22 },
        { id: "gallery", type: "gallery", label: "Galerie", enabled: true, height: 300, radius: 18, padding: 22 },
        { id: "faq", type: "faq", label: "FAQ", enabled: true, height: 280, radius: 18, padding: 22 },
        { id: "cta", type: "cta", label: "Appel à l'action", enabled: true, height: 220, radius: 18, padding: 28 },
        { id: "footer", type: "footer", label: "Footer premium", enabled: true, height: 300, radius: 0, padding: 28 },
      ],
      selectedProducts: [],
      selectedCollections: [],
      bundles: [
      ...
      shipping: { mode: "global", globalLocal: Number(workspace.frais_livraison || 0), globalShipping: Number(workspace.frais_expedition || 0), products: {} },
      header: { showSearch: true, showCart: true, showMenu: true, sticky: true },
      footer: { phone: workspace.whatsapp_number || "", whatsapp: workspace.whatsapp_number || "", facebook: workspace.facebook_url || "", instagram: workspace.instagram_url || "", tiktok: workspace.tiktok_url || "", links: ["Livraison", "Retours", "Confidentialité"] },
    };
  });
  ...
  function moveSection(id, dir) {
    setConfig(c => { const arr=[...c.sections]; const i=arr.findIndex(s=>s.id===id); const j=i+dir; if(i<0||j<0||j>=arr.length)return c; [arr[i],arr[j]]=[arr[j],arr[i]]; return {...c,sections:arr}; });
  }
  function toggleInArray(key,id) { setConfig(c => ({...c,[key]:(c[key]||[]).includes(id)?c[key].filter(x=>x!==id):[...(c[key]||[]),id]})); }
  ...
  function addBundle() { setConfig(c=>({...c,bundles:[...(c.bundles||[]),{id:`b${Date.now()}`,productId:c.selectedProducts?.[0]||"",title:`Pack ×${(c.bundles?.length||0)+1}`,quantity:(c.bundles?.length||0)+1,discount:10,badge:""}]})); }
  function removeBundle(id) { setConfig(c=>({...c,bundles:c.bundles.filter(b=>b.id!==id)})); }
  function shippingFor(productId) { const p=config.shipping?.products?.[productId]; return p || { local:Number(config.shipping?.globalLocal||0), shipping:Number(config.shipping?.globalShipping||0), useGlobal:true }; }
  function updateProductShipping(productId, patch) { setConfig(c=>({...c,shipping:{...(c.shipping||{}),products:{...((c.shipping||{}).products||{}),[productId]:{...shippingFor(productId),...patch,useGlobal:false}}}})); }
  async function uploadStoreImage(kind,file) {
    ...
    setUploading(true);
    const ext=file.name.split(".").pop()||"jpg"; const path=`${workspace.id}/builder-${kind}-${Date.now()}.${ext}`;
    ...
    if(kind==="logo")updateConfig({logo:data.publicUrl}); else if(kind==="hero")updateConfig({heroImage:data.publicUrl}); else setConfig(c=>({...c,gallery:[...(c.gallery||[]),data.publicUrl]}));
    setUploading(false);
  }
  async function save() {
    setSaving(true);
    try {
      ...
      await supabase.from("workspaces").update({logo_url:config.logo||null,banniere_url:config.heroImage||null,couleur_marque:config.primary,description_boutique:config.description,frais_livraison:Number(config.shipping?.globalLocal||0),frais_expedition:Number(config.shipping?.globalShipping||0)}).eq("id",workspace.id);
      alert("✅ Boutique enregistrée.");
    } finally { setSaving(false); }
  }

  const activityLabel={ecommerce:"E-commerce",commerce:"Commerce",restaurant:"Restaurant",location_immobiliere:"Immobilier",location_vehicule:"Location de véhicules"}[activity]||"Activité";
  ...
  const pill={border:0,borderRadius:10,padding:"9px 12px",fontWeight:800,cursor:"pointer"};

  function MediaUpload({kind,labelText,children}) { return <label style={{display:"block",border:"2px dashed #B9CDBF",borderRadius:14,padding:16,textAlign:"center",cursor:"pointer",color:config.primary,fontWeight:900,background:"#F8FBF9"}}>{children||`📤 ${labelText}`}<input type="file" accept="image/*" hidden onChange={e=>uploadStoreImage(kind,e.target.files?.[0])}/></label>; }
  function SizeControls() {
    const s=selected||{}; return <div style={{...card,background:"#F8FAF8"}}><b>📐 Dimension de la section</b><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginTop:10}}><div><label style={label}>Hauteur</label><input type="range" min="120" max="720" value={s.height||300} onChange={e=>updateSection(s.id,{height:Number(e.target.value)})} style={{width:"100%"}}/><div style={{fontSize:12,color:"#6B776F"}}>{s.height||300}px</div></div><div><label style={label}>Arrondi</label><input type="range" min="0" max="40" value={s.radius||0} onChange={e=>updateSection(s.id,{radius:Number(e.target.value)})} style={{width:"100%"}}/><div style={{fontSize:12,color:"#6B776F"}}>{s.radius||0}px</div></div></div><label style={label}>Espacement interne</label><input type="range" min="8" max="56" value={s.padding||20} onChange={e=>updateSection(s.id,{padding:Number(e.target.value)})} style={{width:"100%"}}/><div style={{fontSize:12,color:"#6B776F"}}>{s.padding||20}px — utilisable aussi depuis téléphone</div></div>;
  }

  function renderEditor(){
    if(!selected)return null;
    const controls=<SizeControls/>;
    if(selected.id==="header") return <div><h2 style={{marginTop:0}}>🧭 Header premium</h2><p style={{color:"#65736B"}}>Un header propre, rapide et pensé pour mobile.</p><label style={label}>Style</label><select style={input} value={config.header.style||"premium"} onChange={e=>setNested("header",{style:e.target.value})}><option value="premium">Premium</option><option value="minimal">Minimal</option><option value="dark">Dark</option></select>{[["showSearch","Afficher la recherche"],["showCart","Afficher le panier"],["showMenu","Afficher le menu mobile"],["sticky","Header fixe au défilement"]].map(([k,t])=><label key={k} style={{display:"flex",gap:10,alignItems:"center",padding:"12px 0",fontWeight:800}}><input type="checkbox" checked={config.header?.[k]!==false} onChange={e=>setNested("header",{[k]:e.target.checked})}/>{t}</label>)}{controls}</div>;
    if(selected.id==="hero") return <div><h2 style={{marginTop:0}}>🖼️ Hero / couverture</h2><label style={label}>Titre</label><input style={input} value={config.heroTitle} onChange={e=>updateConfig({heroTitle:e.target.value})}/><label style={label}>Texte</label><textarea style={{...input,minHeight:90}} value={config.heroText} onChange={e=>updateConfig({heroText:e.target.value})}/><label style={label}>Bouton</label><input style={input} value={config.heroButton} onChange={e=>updateConfig({heroButton:e.target.value})}/><label style={label}>Couverture</label>{config.heroImage&&<img src={config.heroImage} alt="Couverture" style={{width:"100%",height:170,objectFit:"cover",borderRadius:14,marginBottom:8}}/>}<MediaUpload kind="hero" labelText="Télécharger / changer la couverture" />{controls}</div>;
    if(selected.id==="announcement") return <div><h2 style={{marginTop:0}}>📢 Barre d'annonce</h2><label style={label}>Message</label><input style={input} value={config.announcement} onChange={e=>updateConfig({announcement:e.target.value})}/>{controls}</div>;
    if(selected.id==="collections") return <div><h2 style={{marginTop:0}}>🗂️ Collections</h2><p style={{color:"#65736B"}}>Sélectionne exactement les collections visibles.</p>{collections.length===0?<div style={card}>Aucune collection disponible.</div>:collections.map(c=><button key={c.id} onClick={()=>toggleInArray("selectedCollections",c.id)} style={{...card,width:"100%",textAlign:"left",cursor:"pointer",borderColor:(config.selectedCollections||[]).includes(c.id)?config.primary:"#DCE6DF",background:(config.selectedCollections||[]).includes(c.id)?"#F1F8F3":"#fff"}}><b>{(config.selectedCollections||[]).includes(c.id)?"✓":"○"} {c.nom}</b><span style={{float:"right",color:"#78847D"}}>{(collectionProducts[c.id]||[]).length} produits</span></button>)}{controls}</div>;
    if(selected.id==="catalog"||selected.id==="best") return <div><h2 style={{marginTop:0}}>{selected.id==="best"?"🔥 Meilleures ventes":"🛍️ Catalogue produits"}</h2><p style={{color:"#65736B"}}>Clique sur les produits pour les sélectionner. Ces produits restent ceux de ton espace RecuVente.</p>{products.length===0?<div style={card}>Aucun produit trouvé. Tu peux importer ton CSV depuis <b>Produits → Importer un catalogue</b>.</div>:products.map(p=>{const active=(config.selectedProducts||[]).includes(p.id);return <button key={p.id} onClick={()=>toggleInArray("selectedProducts",p.id)} style={{...card,width:"100%",display:"flex",alignItems:"center",gap:12,textAlign:"left",cursor:"pointer",borderColor:active?config.primary:"#DCE6DF",background:active?"#F3F9F4":"#fff"}}>{p.photo_url?<img src={p.photo_url} alt="" style={{width:58,height:58,objectFit:"cover",borderRadius:10}}/>:<div style={{width:58,height:58,borderRadius:10,background:"#EDF3EF",display:"grid",placeItems:"center"}}>📦</div>}<span style={{flex:1}}><b>{p.nom}</b><br/><small>{Number(p.prix_vente||p.prix||0).toLocaleString("fr-FR")} {workspace.currency||"XOF"}</small></span><b style={{fontSize:20,color:active?config.primary:"#A6B0AA"}}>{active?"✓":"○"}</b></button>})}{controls}</div>;
    if(selected.id==="bundles") return <div><h2 style={{marginTop:0}}>📦 Packs / Bundles par produit</h2><p style={{color:"#65736B"}}>Chaque pack doit être lié à un produit. Si un produit n'a aucun pack, aucun bundle ne lui est affiché.</p>{config.bundles.map(b=><div key={b.id} style={card}><label style={label}>Produit concerné</label><select style={input} value={b.productId||""} onChange={e=>updateBundle(b.id,{productId:e.target.value})}><option value="">— Sélectionner un produit —</option>{products.map(p=><option key={p.id} value={p.id}>{p.nom} — {Number(p.prix_vente||p.prix||0).toLocaleString("fr-FR")} {workspace.currency||"XOF"}</option>)}</select><div style={{display:"grid",gridTemplateColumns:"1fr 100px",gap:10}}><div><label style={label}>Nom du pack</label><input style={input} value={b.title} onChange={e=>updateBundle(b.id,{title:e.target.value})}/></div><div><label style={label}>Quantité</label><input style={input} type="number" min="1" value={b.quantity} onChange={e=>updateBundle(b.id,{quantity:Math.max(1,Number(e.target.value))})}/></div></div><label style={label}>Remise (%)</label><input style={input} type="number" min="0" max="90" value={b.discount} onChange={e=>updateBundle(b.id,{discount:Math.min(90,Math.max(0,Number(e.target.value)))})}/><label style={label}>Badge</label><input style={input} value={b.badge} onChange={e=>updateBundle(b.id,{badge:e.target.value})}/><button onClick={()=>removeBundle(b.id)} style={{...pill,background:"#FFF1F0",color:"#A13B35",marginTop:10}}>Supprimer ce pack</button></div>)}<button onClick={addBundle} style={{...pill,width:"100%",background:config.primary,color:"white"}}>＋ Ajouter un pack</button>{controls}</div>;
    if(selected.id==="shipping") return <div><h2 style={{marginTop:0}}>🚚 Livraison & expédition</h2><div style={{...card,background:"#F5FAF6"}}><b>Règle générale</b><p style={{margin:"6px 0",fontSize:13,color:"#65736B"}}>Ces frais s'appliquent par défaut à tous les produits.</p><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><div><label style={label}>Livraison locale / Abidjan</label><input style={input} type="number" min="0" value={config.shipping?.globalLocal||0} onChange={e=>setConfig(c=>({...c,shipping:{...(c.shipping||{}),globalLocal:Number(e.target.value)}}))}/></div><div><label style={label}>Expédition autre ville</label><input style={input} type="number" min="0" value={config.shipping?.globalShipping||0} onChange={e=>setConfig(c=>({...c,shipping:{...(c.shipping||{}),globalShipping:Number(e.target.value)}}))}/></div></div></div><h3>⚙️ Exceptions par produit</h3><p style={{fontSize:13,color:"#65736B"}}>Pour chaque produit : utiliser la règle générale, définir ses propres frais ou désactiver la livraison/expédition.</p>{products.map(p=>{const sp=shippingFor(p.id);return <div key={p.id} style={card}><b>{p.nom}</b><label style={{display:"flex",gap:8,alignItems:"center",marginTop:10,fontWeight:800}}><input type="checkbox" checked={sp.useGlobal!==false} onChange={e=>updateProductShipping(p.id,{useGlobal:e.target.checked})}/> Utiliser les frais généraux</label>{sp.useGlobal===false&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><div><label style={label}>Livraison locale</label><input style={input} type="number" min="0" value={sp.local||0} onChange={e=>updateProductShipping(p.id,{local:Number(e.target.value)})}/></div><div><label style={label}>Expédition</label><input style={input} type="number" min="0" value={sp.shipping||0} onChange={e=>updateProductShipping(p.id,{shipping:Number(e.target.value)})}/></div></div>}<label style={{display:"flex",gap:8,alignItems:"center",marginTop:10,fontWeight:800}}><input type="checkbox" checked={sp.local!==null} onChange={e=>updateProductShipping(p.id,{local:e.target.checked?(sp.local||Number(config.shipping?.globalLocal||0)):null})}/> Livraison locale autorisée</label><label style={{display:"flex",gap:8,alignItems:"center",marginTop:8,fontWeight:800}}><input type="checkbox" checked={sp.shipping!==null} onChange={e=>updateProductShipping(p.id,{shipping:e.target.checked?(sp.shipping||Number(config.shipping?.globalShipping||0)):null})}/> Expédition autorisée</label></div>})}{controls}</div>;
    if(selected.id==="gallery") return <div><h2 style={{marginTop:0}}>🖼️ Galerie</h2><MediaUpload kind="gallery">{uploading?"Envoi en cours…":"📤 Ajouter une image"}</MediaUpload><div style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:10,marginTop:12}}>{(config.gallery||[]).map((g,i)=><div key={i} style={{position:"relative"}}><img src={g} alt="" style={{width:"100%",height:130,objectFit:"cover",borderRadius:12}}/><button onClick={()=>setConfig(c=>({...c,gallery:c.gallery.filter((_,x)=>x!==i)}))} style={{position:"absolute",top:6,right:6,border:0,borderRadius:99,width:30,height:30,cursor:"pointer"}}>×</button></div>)}</div>{controls}</div>;
    if(selected.id==="footer") return <div><h2 style={{marginTop:0}}>🧭 Footer premium</h2><p style={{color:"#65736B"}}>Un footer complet : contact, réseaux, navigation et réassurance.</p>{[["phone","Téléphone"],["whatsapp","WhatsApp"],["facebook","Facebook"],["instagram","Instagram"],["tiktok","TikTok"]].map(([k,t])=><div key={k}><label style={label}>{t}</label><input style={input} value={config.footer?.[k]||""} onChange={e=>setNested("footer",{[k]:e.target.value})}/></div>)}<label style={label}>Liens rapides (séparés par des virgules)</label><input style={input} value={(config.footer?.links||[]).join(", ")} onChange={e=>setNested("footer",{links:e.target.value.split(",").map(x=>x.trim()).filter(Boolean)})}/>{controls}</div>;
    if(selected.id==="promo") return <div><h2 style={{marginTop:0}}>🏷️ Promotion</h2><label style={label}>Titre</label><input style={input} value={config.promoTitle} onChange={e=>updateConfig({promoTitle:e.target.value})}/><label style={label}>Texte</label><textarea style={{...input,minHeight:90}} value={config.promoText} onChange={e=>updateConfig({promoText:e.target.value})}/>{controls}</div>;
    if(selected.id==="cta") return <div><h2 style={{marginTop:0}}>🎯 Appel à l'action</h2><label style={label}>Titre</label><input style={input} value={config.ctaTitle} onChange={e=>updateConfig({ctaTitle:e.target.value})}/><label style={label}>Texte</label><textarea style={{...input,minHeight:90}} value={config.ctaText} onChange={e=>updateConfig({ctaText:e.target.value})}/>{controls}</div>;
    return <div><h2 style={{marginTop:0}}>✏️ {selected.label}</h2><p style={{color:"#65736B"}}>Cette section est activée et personnalisable.</p>{controls}</div>;
  }

  function Preview(){
    const mobile=device==="mobile"; const tablet=device==="tablet"; const width=mobile?390:tablet?820:1180;
    const enabled=config.sections.filter(s=>s.enabled);
    const Header=()=> <div style={{height:config.sections.find(s=>s.id==="header")?.height||72,background:config.header?.style==="dark"?"#10231A":"#fff",color:config.header?.style==="dark"?"#fff":"#17241E",display:"flex",alignItems:"center",gap:14,padding:"0 18px",position:config.header?.sticky?"sticky":"relative",top:0,zIndex:5,borderBottom:"1px solid #E6ECE8",boxShadow:"0 4px 18px rgba(16,35,26,.06)"}}><div style={{display:"flex",alignItems:"center",gap:9,flex:1,minWidth:0}}>{config.logo?<img src={config.logo} alt="" style={{width:40,height:40,objectFit:"cover",borderRadius:10}}/>:<div style={{width:40,height:40,borderRadius:10,background:config.primary,display:"grid",placeItems:"center",color:"#fff",fontWeight:900}}>R</div>}<b style={{fontSize:mobile?16:18,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{config.name}</b></div>{config.header?.showSearch&&<div style={{display:mobile?"none":"block",padding:"9px 14px",border:"1px solid #DDE5DF",borderRadius:99,color:"#8A948E",minWidth:150}}>🔎 Rechercher</div>}{config.header?.showCart&&<div style={{fontSize:20}}>🛒</div>}{config.header?.showMenu&&<div style={{fontSize:22}}>☰</div>}</div>;
    return <div style={{minHeight:"100%",display:"flex",justifyContent:"center",padding:mobile?8:18,background:"#E9EEEB"}}><div style={{width:"100%",maxWidth:width,background:"#fff",boxShadow:"0 20px 50px rgba(16,35,26,.12)",overflow:"hidden",borderRadius:mobile?16:22}}>{enabled.map(s=><div key={s.id} style={{minHeight:s.height||200,borderRadius:s.radius||0,overflow:"hidden"}}>{s.id==="header"&&<Header/>}{s.id==="announcement"&&<div style={{minHeight:s.height||42,padding:s.padding||10,background:config.primary,color:"white",display:"grid",placeItems:"center",fontWeight:800,fontSize:mobile?11:13}}>{config.announcement}</div>}{s.id==="hero"&&<div style={{minHeight:s.height||440,padding:s.padding||28,display:"flex",alignItems:"center",background:config.heroImage?`linear-gradient(90deg,rgba(16,35,26,.78),rgba(16,35,26,.18)),url(${config.heroImage}) center/cover`:"linear-gradient(135deg,#10231A,#1a7a3c)",color:"white"}}><div style={{maxWidth:650}}><div style={{fontSize:mobile?11:13,fontWeight:900,textTransform:"uppercase",letterSpacing:1.5,opacity:.8,marginBottom:10}}>RECUVENTE STORE</div><h1 style={{fontSize:mobile?32:52,lineHeight:1.03,margin:"0 0 14px"}}>{config.heroTitle}</h1><p style={{fontSize:mobile?14:18,lineHeight:1.55,opacity:.9}}>{config.heroText}</p><button style={{marginTop:14,background:"#fff",color:config.primary,border:0,borderRadius:12,padding:"13px 18px",fontWeight:900}}>{config.heroButton}</button></div></div>}{s.id==="collections"&&<div style={{padding:s.padding||22}}><h2>🗂️ Collections</h2><div style={{display:"grid",gridTemplateColumns:mobile?"repeat(2,1fr)":"repeat(4,1fr)",gap:10}}>{collections.filter(c=>(config.selectedCollections||[]).includes(c.id)).map(c=><div key={c.id} style={{padding:18,border:"1px solid #E0E8E2",borderRadius:14,fontWeight:900}}>{c.nom}</div>)}{!(config.selectedCollections||[]).length&&<div style={{color:"#8A958E"}}>Sélectionne tes collections dans l'éditeur.</div>}</div></div>}{(s.id==="catalog"||s.id==="best")&&<div style={{padding:s.padding||22}}><h2>{s.id==="best"?"🔥 Meilleures ventes":"🛍️ Nos produits"}</h2><div style={{display:"grid",gridTemplateColumns:mobile?"repeat(2,1fr)":tablet?"repeat(3,1fr)":"repeat(4,1fr)",gap:10}}>{selectedProducts.map(p=><div key={p.id} style={{border:"1px solid #E1E8E3",borderRadius:14,overflow:"hidden"}}>{p.photo_url?<img src={p.photo_url} alt="" style={{width:"100%",height:mobile?130:180,objectFit:"cover"}}/>:<div style={{height:mobile?130:180,background:"#F0F4F1",display:"grid",placeItems:"center"}}>📦</div>}<div style={{padding:10}}><b style={{fontSize:13}}>{p.nom}</b><div style={{marginTop:5,fontWeight:900,color:config.primary}}>{Number(p.prix_vente||p.prix||0).toLocaleString("fr-FR")} {workspace.currency||"XOF"}</div></div></div>)}{!selectedProducts.length&&<div style={{gridColumn:"1/-1",color:"#89958F"}}>Sélectionne tes produits dans l'éditeur.</div>}</div></div>}{s.id==="bundles"&&<div style={{padding:s.padding||22,background:"#FAFCFA"}}><h2>📦 Choisis ton pack</h2><div style={{display:"grid",gridTemplateColumns:mobile?"1fr":tablet?"repeat(2,1fr)":"repeat(3,1fr)",gap:10}}>{config.bundles.filter(b=>b.productId&&selectedProducts.some(p=>p.id===b.productId)).map(b=>{const p=products.find(x=>x.id===b.productId);const price=Number(p?.prix_vente||p?.prix||0);const total=price*b.quantity*(1-b.discount/100);return <div key={b.id} style={{border:`2px solid ${b.discount>=10?config.primary:"#E1E8E3"}`,borderRadius:14,padding:14,position:"relative"}}>{b.badge&&<span style={{position:"absolute",top:-9,right:8,background:config.primary,color:"white",padding:"4px 8px",borderRadius:99,fontSize:10,fontWeight:900}}>{b.badge}</span>}<b>{b.title}</b><div style={{fontSize:12,color:"#6C7771",marginTop:5}}>{p?.nom}</div><div style={{fontSize:20,fontWeight:900,marginTop:8}}>{total.toLocaleString("fr-FR")} {workspace.currency||"XOF"}</div></div>})}</div></div>}{s.id==="shipping"&&<div style={{padding:s.padding||22}}><h2>🚚 Livraison & paiement</h2><div style={{display:"grid",gridTemplateColumns:mobile?"1fr":"1fr 1fr",gap:10}}><div style={{padding:16,borderRadius:14,background:"#F5FAF6"}}><b>📍 Abidjan / local</b><div style={{fontSize:20,fontWeight:900,marginTop:6}}>{Number(config.shipping?.globalLocal||0).toLocaleString("fr-FR")} {workspace.currency||"XOF"}</div></div><div style={{padding:16,borderRadius:14,background:"#F5FAF6"}}><b>📦 Expédition</b><div style={{fontSize:20,fontWeight:900,marginTop:6}}>{Number(config.shipping?.globalShipping||0).toLocaleString("fr-FR")} {workspace.currency||"XOF"}</div></div></div><p style={{fontSize:13,color:"#65736B"}}>💵 Paiement à la livraison (COD)</p></div>}{s.id==="trust"&&<div style={{padding:s.padding||22,display:"grid",gridTemplateColumns:mobile?"1fr":"repeat(3,1fr)",gap:10}}>{["💵 Paiement à la livraison","🚚 Livraison locale & expédition","💬 Support WhatsApp"].map(x=><div key={x} style={{padding:16,border:"1px solid #E1E8E3",borderRadius:14,fontWeight:900}}>{x}</div>)}</div>}{s.id==="promo"&&<div style={{margin:s.padding||22,padding:24,borderRadius:s.radius||18,background:config.primary,color:"white"}}><h2>{config.promoTitle}</h2><p>{config.promoText}</p></div>}{s.id==="reviews"&&<div style={{padding:s.padding||22}}><h2>⭐ Avis clients</h2><div style={{padding:16,background:"#F5F8F5",borderRadius:12}}>« Une commande simple et rapide. »<br/><small>Client vérifié</small></div></div>}{s.id==="gallery"&&<div style={{padding:s.padding||22}}><h2>Galerie</h2><div style={{display:"grid",gridTemplateColumns:mobile?"repeat(2,1fr)":"repeat(4,1fr)",gap:8}}>{(config.gallery||[]).map(g=><img key={g} src={g} alt="" style={{width:"100%",height:mobile?110:150,objectFit:"cover",borderRadius:10}}/>)}</div></div>}{s.id==="faq"&&<div style={{padding:s.padding||22}}><h2>Questions fréquentes</h2><details><summary style={{fontWeight:900}}>Comment commander ?</summary><p>Choisis ton produit, ta quantité et remplis le bon de commande.</p></details><details><summary style={{fontWeight:900}}>Comment payer ?</summary><p>Paiement à la livraison (COD).</p></details></div>}{s.id==="cta"&&<div style={{padding:s.padding||28,textAlign:"center",background:"#F1F6F2"}}><h2>{config.ctaTitle}</h2><p>{config.ctaText}</p><button style={{background:config.primary,color:"white",border:0,borderRadius:12,padding:"13px 20px",fontWeight:900}}>Commander maintenant</button></div>}{s.id==="footer"&&<div style={{padding:s.padding||28,background:"#10231A",color:"white"}}><div style={{display:"grid",gridTemplateColumns:mobile?"1fr":"2fr 1fr 1fr",gap:24}}><div><div style={{display:"flex",alignItems:"center",gap:9}}>{config.logo?<img src={config.logo} alt="" style={{width:42,height:42,objectFit:"cover",borderRadius:10}}/>:<div style={{width:42,height:42,borderRadius:10,background:config.primary,display:"grid",placeItems:"center"}}>R</div>}<b style={{fontSize:18}}>{config.name}</b></div><p style={{opacity:.72,lineHeight:1.6}}>{config.description}</p></div><div><b>Navigation</b>{(config.footer?.links||[]).map(x=><div key={x} style={{opacity:.72,marginTop:9}}>{x}</div>)}</div><div><b>Contact</b><div style={{opacity:.72,marginTop:9}}>{config.footer?.phone||"Téléphone"}</div><div style={{opacity:.72,marginTop:7}}>{config.footer?.whatsapp?`WhatsApp : ${config.footer.whatsapp}`:"Support WhatsApp"}</div></div></div><div style={{borderTop:"1px solid rgba(255,255,255,.12)",marginTop:24,paddingTop:16,fontSize:11,opacity:.55}}>© {new Date().getFullYear()} {config.name} • Paiement à la livraison • Boutique propulsée par RecuVente</div></div>}</div>)}</div></div>;
  }
  ...
  return <div style={{position:"fixed",inset:0,zIndex:100,background:"#E9EEEB",display:"flex",flexDirection:"column",fontFamily:"Inter,system-ui,sans-serif"}}>
    <div style={{minHeight:64,background:"#10231A",color:"white",display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,padding:"10px 14px",flexShrink:0}}><div style={{minWidth:0}}><b style={{fontSize:17}}>🛍️ Constructeur de boutique</b><span style={{marginLeft:8,fontSize:11,background:"#DDF2E2",color:"#176535",padding:"5px 8px",borderRadius:99}}>{activityLabel}</span></div><div style={{display:"flex",gap:7}}><button onClick={save} disabled={saving} style={{...pill,background:"#1a7a3c",color:"white"}}>{saving?"Enregistrement…":"💾 Enregistrer"}</button><button onClick={onClose} style={{...pill,background:"rgba(255,255,255,.12)",color:"white"}}>Fermer</button></div></div>
    <div style={{display:"flex",gap:7,padding:8,background:"white",borderBottom:"1px solid #DDE5DF",overflowX:"auto",flexShrink:0}}>{[["structure","🧱 Sections"],["edit","✏️ Modifier"],["preview","👁 Aperçu"]].map(([id,t])=><button key={id} onClick={()=>setTab(id)} style={{whiteSpace:"nowrap",border:0,borderRadius:10,padding:"10px 13px",background:tab===id?"#E5F2E8":"#F5F7F6",color:tab===id?config.primary:"#59665F",fontWeight:900,cursor:"pointer"}}>{t}</button>)}</div>
    {tab==="preview"?<><div style={{display:"flex",justifyContent:"center",gap:7,padding:8,background:"#F7F9F7",borderBottom:"1px solid #DDE5DF"}}>{[["desktop","🖥️ Desktop"],["tablet","▣ Tablette"],["mobile","📱 Mobile"]].map(([d,t])=><button key={d} onClick={()=>setDevice(d)} style={{...pill,background:device===d?config.primary:"white",color:device===d?"white":"#4F5E56"}}>{t}</button>)}</div><div style={{flex:1,overflow:"auto"}}><Preview/></div></>:<div style={{flex:1,minHeight:0,display:"flex",flexDirection:device==="mobile"?"column":"row"}}>
      <aside style={{width:device==="mobile"?"100%":320,maxHeight:device==="mobile"?"44vh":"none",background:"#F7F9F7",borderRight:device==="mobile"?"none":"1px solid #DDE5DF",borderBottom:device==="mobile"?"1px solid #DDE5DF":"none",padding:10,overflowY:"auto",flexShrink:0}}><div style={{fontSize:11,fontWeight:900,color:"#718078",textTransform:"uppercase",marginBottom:8}}>Structure — touche une section</div>{config.sections.map(s=><div key={s.id} style={{display:"flex",gap:5,alignItems:"center",marginBottom:6}}><button onClick={()=>{setSelectedSection(s.id);setTab("edit")}} style={{flex:1,textAlign:"left",padding:"12px 10px",borderRadius:11,border:s.id===selectedSection?`2px solid ${config.primary}`:"1px solid #E0E7E2",background:s.enabled?"white":"#EDF0EE",color:s.enabled?"#16231F":"#89938E",fontWeight:900,cursor:"pointer"}}>{s.enabled?"●":"○"} {s.label}</button><button onClick={()=>moveSection(s.id,-1)} style={{border:0,background:"white",padding:8,borderRadius:8,cursor:"pointer"}}>↑</button><button onClick={()=>moveSection(s.id,1)} style={{border:0,background:"white",padding:8,borderRadius:8,cursor:"pointer"}}>↓</button></div>)}</aside>
      <main style={{flex:1,overflowY:"auto",padding:device==="mobile"?12:18}}><div style={{maxWidth:760,margin:"0 auto"}}>{tab==="structure"?<div><div style={card}><h2 style={{marginTop:0}}>Ta boutique, ton design</h2><p style={{color:"#65736B",lineHeight:1.6}}>Sur téléphone aussi : touche une section, modifie-la et utilise les curseurs pour l'étirer ou réduire sa hauteur et ses espaces.</p><div style={{display:"flex",gap:8,flexWrap:"wrap"}}><button onClick={()=>setTab("preview")} style={{...pill,background:config.primary,color:"white"}}>👁 Voir la boutique</button><button onClick={()=>setTab("edit")} style={{...pill,background:"#F0F5F1",color:config.primary}}>✏️ Modifier {selected?.label}</button></div></div><div style={card}><label style={label}>Nom de la boutique</label><input style={input} value={config.name} onChange={e=>updateConfig({name:e.target.value})}/><label style={label}>Description</label><textarea style={{...input,minHeight:80}} value={config.description} onChange={e=>updateConfig({description:e.target.value})}/><label style={label}>Couleur principale</label><div style={{display:"flex",gap:10,alignItems:"center"}}><input type="color" value={config.primary} onChange={e=>updateConfig({primary:e.target.value})} style={{width:50,height:44,border:0}}/><input style={input} value={config.primary} onChange={e=>updateConfig({primary:e.target.value})}/></div><label style={label}>Logo</label>{config.logo&&<img src={config.logo} alt="Logo" style={{width:64,height:64,objectFit:"cover",borderRadius:12,marginBottom:8}}/>}<MediaUpload kind="logo">{uploading?"Envoi en cours…":"📤 Télécharger / changer le logo"}</MediaUpload></div></div>:<div><button onClick={()=>setTab("structure")} style={{border:0,background:"transparent",color:config.primary,fontWeight:900,padding:"2px 0 14px",cursor:"pointer"}}>← Retour aux sections</button>{renderEditor()}</div>}</div></main>
    </div>}
  </div>;
}
