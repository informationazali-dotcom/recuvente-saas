...
function RVStoreBuilder({ workspace, produits = [], clients = [], onClose, onOuvrirParametresAvances }) {
  ...
  async function uploadImage(kind,file){
    ...
    if(kind==='hero') update('banniere',url); else if(kind==='logo') update('logo',url); else if(kind==='gallery') setConfig(c=>({...c,gallery:[...(c.gallery||[]),url]})); else if(kind==='imageTexte') update('imageTexteImage',url);
    setUploading(null);
  }
  async function save(){
  ...
  function PreviewSection({type}){
    ...
    if(type==='image_texte'){const inverse=config.imageTextePosition==='droite';return <div style={{...common,padding:0}}><div style={{display:'flex',flexDirection:device==='mobile'?'column':(inverse?'row-reverse':'row')}}><div style={{flex:1,minHeight:device==='mobile'?160:220,background:config.imageTexteImage?`url(${config.imageTexteImage}) center/cover`:`linear-gradient(135deg,${coul},#0b2416)`}}/><div style={{flex:1,padding:'26px 22px',display:'flex',flexDirection:'column',justifyContent:'center'}}><div style={{fontSize:device==='mobile'?18:22,fontWeight:900,color:'#132019',marginBottom:8}}>{config.imageTexteTitre}</div><div style={{fontSize:12,color:'#68756d',lineHeight:1.65}}>{config.imageTexteTexte}</div></div></div></div>;}
  ...
  function EditorInterne(){
    ...
    if(type==='image_texte')return <><label style={labelStyle}>Titre<input style={fieldStyle} value={config.imageTexteTitre} onChange={e=>update('imageTexteTitre',e.target.value)}/></label><label style={labelStyle}>Texte<textarea style={{...fieldStyle,resize:'vertical'}} rows={5} value={config.imageTexteTexte} onChange={e=>update('imageTexteTexte',e.target.value)}/></label><label style={labelStyle}>Position de l'image<select style={fieldStyle} value={config.imageTextePosition} onChange={e=>update('imageTextePosition',e.target.value)}><option value="gauche">Image à gauche</option><option value="droite">Image à droite</option></select></label>{config.imageTexteImage&&<img src={config.imageTexteImage} alt="" style={{width:'100%',height:80,objectFit:'cover',borderRadius:10,marginBottom:8}}/>}<FileButton kind="imageTexte" label="Télécharger / changer l'image"/><label style={{...labelStyle,marginTop:8}}>Ou URL de l'image<input style={fieldStyle} placeholder="https://..." value={config.imageTexteImage} onChange={e=>update('imageTexteImage',e.target.value)}/></label></>;
