...
function LandingPage() {
  ...
  const liveStats = [
    ...
    ['COMMERCE CONFIRMÉ', statValue('montant_total_confirme'), 'FCFA'],
  ...
  return (
    <div className="rva">
      ...
      <main id="top">
        <section className="rva-hero"><div className="rva-glow"/><div className="wrap rva-heroGrid">
          <div className="rva-heroCopy"><div className="rva-kicker"><b/> LA PLATEFORME POUR PILOTER VOTRE ACTIVITÉ</div><h1 className="rva-h1">Votre activité.<span>Enfin sous contrôle.</span></h1><p>Vous n'avez pas besoin d'un outil de plus. Vous avez besoin d'un système qui relie <b>vente, équipe, opérations, boutique, encaissement et performance</b> pour vous permettre de décider avec une vision claire.</p><div className="rva-actions"><a className="rva-btn primary" href="#tarifs" onClick={()=>trackLead('ViewPricing')}>Choisir mon abonnement →</a><a className="rva-btn ghost" href="#boutique">Voir le système en action</a></div><div className="rva-proof"><span>✓ <b>Données réelles</b> affichées quand disponibles</span><span>✓ <b>Plans payants</b> issus de votre configuration</span></div></div>
          <div className="rva-stage"><div className="rva-orbit"/><div className="rva-dashboard"><div className="rva-dbtop"><div className="rva-dbbrand">RECUVENTE<small>PILOTAGE CENTRAL</small></div><span className="rva-live">● DONNÉES DISPONIBLES</span></div><div className="rva-dbgrid">{[['COMMERCE CONFIRMÉ',statValue('montant_total_confirme'),'FCFA'],['COMMANDES',statValue('nb_commandes_confirmees'),'confirmées'],['ENTREPRISES',statValue('nb_entreprises_actives'),'actives']].map(x=><div className="rva-kpi" key={x[0]}><small>{x[0]}</small><strong>{x[1]}</strong><em>{x[2]}</em></div>)}</div><div className="rva-dbmain"><div className="rva-chart"><div className="rva-charthead"><span>Évolution du commerce</span><span>LIVE</span></div><svg viewBox="0 0 500 160" preserveAspectRatio="none"><path className="area" d="M0 140 L70 118 L140 128 L205 83 L270 96 L335 55 L400 72 L500 22 L500 160 L0 160 Z"/><path d="M0 140 L70 118 L140 128 L205 83 L270 96 L335 55 L400 72 L500 22"/></svg></div><div className="rva-side"><h4>Opérations</h4><div className="rva-mini">{[42,61,48,76,68,88].map((h,i)=><i key={i} style={{height:`${h}%`}}/>)}</div></div></div></div><div className="rva-float one"><small>COMMERCE CONFIRMÉ</small><strong>{statValue('montant_total_confirme')} <b>FCFA</b></strong></div><div className="rva-float two"><small>ENTREPRISES ACTIVES</small><strong>{statValue('nb_entreprises_actives')}</strong></div><div className="rva-float three"><small>LIVREURS ACTIFS</small><strong>{statValue('nb_livreurs_actifs')}</strong></div><div className="rva-3dbadge">UNE VISION. UNE ÉQUIPE. UN SYSTÈME.</div></div>
        </div></section>
