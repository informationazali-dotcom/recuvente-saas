...
function eventPixel(nom, data) {
  if (window.fbq) window.fbq("track", nom, data);
}
...
export default function CataloguePublic({ workspaceId }) {
  ...
  useEffect(() => {
    supabase.rpc("catalogue_public", { p_workspace_id: workspaceId }).then(({ data, error }) => {
      ...
      const infos = {
        ...
        pixelId: data[0].facebook_pixel_id,
      };
      setEntreprise(infos);
      setProduits(data.filter((p) => p.produit_nom));
      chargerPixelFacebook(infos.pixelId);
    });
  }, [workspaceId]);

  function ajouterAuPanier(produit) {
    setPanier((p) => ({ ...p, [produit.produit_id]: (p[produit.produit_id] || 0) + 1 }));
    eventPixel("AddToCart", {
      content_name: produit.produit_nom,
      content_ids: [produit.produit_id],
      value: Number(produit.prix_vente),
  ...
  async function envoyerCommande() {
    ...
    eventPixel("Purchase", {
      value: totalMontant,
      currency: entreprise?.devise || "XOF",
      content_ids: items.map((i) => i.produit_id),
      num_items: totalArticles,
    });
    setEnvoye(true);
  }
  ...
  return (
    <div style={{ background: "#FAFAF7", minHeight: "100vh", fontFamily: "sans-serif", paddingBottom: 90 }}>
      <div style={{ width: "100%", maxWidth: 480, margin: "0 auto" }}>
        ...
        {erreur && (
          <div style={{ background: "white", border: "1px solid #ECE8DC", borderRadius: 16, padding: 26, textAlign: "center", marginTop: 60, marginLeft: 16, marginRight: 16 }}>
        ...
        {entreprise && !erreur && envoye && (
          <div style={{ background: "white", border: "1px solid #ECE8DC", borderRadius: 16, padding: 30, textAlign: "center", marginTop: 60, marginLeft: 16, marginRight: 16 }}>
        ...
        {entreprise && !erreur && !envoye && !vuePanier && (
          <>
            {entreprise.banniere ? (
              <div style={{ width: "100%", height: 140, background: `url(${entreprise.banniere}) center/cover`, borderRadius: "0 0 20px 20px", marginBottom: -34, position: "relative" }} />
            ) : (
              <div style={{ width: "100%", height: 90, background: `linear-gradient(135deg, ${couleur}, ${couleur}dd)`, borderRadius: "0 0 20px 20px", marginBottom: -34 }} />
            )}
            ...
            <div style={{ textAlign: "center", marginBottom: 22, padding: "0 16px" }}>
              {entreprise.logo ? (
                <img
                  src={entreprise.logo}
                  alt={entreprise.nom}
                  style={{ width: 68, height: 68, borderRadius: 18, objectFit: "cover", border: "3px solid white", boxShadow: "0 4px 14px rgba(0,0,0,0.15)", marginBottom: 10, background: "white" }}
                />
              ) : (
                <div style={{ width: 68, height: 68, borderRadius: 18, background: couleur, border: "3px solid white", boxShadow: "0 4px 14px rgba(0,0,0,0.15)", marginBottom: 10, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26, margin: "0 auto 10px" }}>
                  🛍️
                </div>
              )}
              <div style={{ fontWeight: 700, fontSize: 21 }}>{entreprise.nom}</div>
              <div style={{ fontSize: 12.5, color: "#8A9089", marginTop: 4 }}>
                {entreprise.description || "Ajoute des produits à ton panier, puis commande en une fois"}
              </div>
            </div>
                ...
                {produits.map((p) => (
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      {p.produit_description && (
                        <div style={{ fontSize: 11.5, color: "#8A9089", marginTop: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.produit_description}</div>
                      )}
                    ...
                    {panier[p.produit_id] ? (
                      <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
                        ...
                        <button onClick={() => ajouterAuPanier(p)} style={{ width: 28, height: 28, borderRadius: 8, border: "none", background: couleur, color: "white", fontSize: 15, cursor: "pointer" }}>+</button>
                      </div>
                    ) : (
                      <button
