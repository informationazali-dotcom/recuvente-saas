...
export default async function handler(req, res) {
  ...
  if (!RECUVENTE_ADMIN_EMAIL || userData.user.email.toLowerCase() !== RECUVENTE_ADMIN_EMAIL.toLowerCase()) {
  ...
  return res.status(200).json({ workspaces: enrichis, mrr, enEssai, actifs, total: enrichis.length });
}
