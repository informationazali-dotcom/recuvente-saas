...
import { reversementsStripeAdminListe, reversementsStripeMarquerReverse } from "./reversements-stripe.js";
...
export async function traiterAction(req, res) {
  ...
  try {
    ...
    if (
      action === "amb_admin_liste" || action === "amb_admin_payer" || action === "annuaire_admin_liste" || action === "annuaire_une" ||
      action === "stripe_reversements_admin_liste" || action === "stripe_reversements_admin_marquer"
    ) {
      ...
      // --- Reversement Stripe (marché Europe) : voir lib/reversements-stripe.js ---
      if (action === "stripe_reversements_admin_liste") return res.status(200).json(await reversementsStripeAdminListe());
      if (action === "stripe_reversements_admin_marquer") {
        if (!UUID.test(String(corps.workspace_id || ""))) return res.status(400).json({ error: "Boutique invalide" });
        return res.status(200).json(await reversementsStripeMarquerReverse(corps.workspace_id));
      }
