...
async function appliquerPaiement(p, montantRecu, stripeCustomerId) {
  ...
  // Ce que RecuVente doit reverser au commerçant (compte Stripe unique, voir en-tête de fichier) —
  // voir lib/reversements-stripe.js. Jamais laissé bloquer le crédit du client ci-dessus, qui a
  // déjà réussi à ce stade : une erreur ici est juste avalée, comme pour la notification ci-dessus.
  try {
    const reversements = await import("./reversements-stripe.js");
    await reversements.enregistrerReversementStripe({ workspaceId: cmd.workspace_id, paiementEnLigneId: p.id, commandeId: cmd.id, montant: montantRecu, devise: p.devise });
  } catch (_) {}
  if (stripeCustomerId) {
